...
def _evaluate(
    ...
    with torch.inference_mode():
        for batch in tqdm(loader, desc="Evaluating"):
            ...
            if metric is not None:
                ...
                cached.append({"probs": {k: v.cpu() for k, v in probs.items()}, "vad": batch["vad"].cpu()})
    ...
    LABELS = {
        ...
        "bp": ("BC-negative", "BC-positive"),
    }

    def _replay(seed=None):
        if seed is not None:
            random.seed(seed)
        metric.reset()
        for r in cached:
    ...
    # SP and BP: stochastic negative sampling → run with multiple seeds
    sp_acc0s, sp_acc1s, sp_baccs, sp_f1s = [], [], [], []
    bp_acc0s, bp_acc1s, bp_baccs, bp_f1s = [], [], [], []
    for seed in tqdm(range(1, SP_N_SEEDS + 1), desc="SP/BP seeds"):
        scores_s = _replay(seed=seed)
        if "sp" in scores_s:
            s = scores_s["sp"]
            a0, a1 = float(s["acc"][0]), float(s["acc"][1])
            sp_acc0s.append(a0); sp_acc1s.append(a1)
            sp_baccs.append((a0 + a1) / 2.0); sp_f1s.append(float(s["f1"]))
        if "bp" in scores_s:
            b = scores_s["bp"]
            a0, a1 = float(b["acc"][0]), float(b["acc"][1])
            bp_acc0s.append(a0); bp_acc1s.append(a1)
            bp_baccs.append((a0 + a1) / 2.0); bp_f1s.append(float(b["f1"]))

    if sp_baccs:
        lbl0, lbl1 = LABELS["sp"]
        print(f"SP:  {lbl0}={np.mean(sp_acc0s):.4f}±{np.std(sp_acc0s):.4f}  {lbl1}={np.mean(sp_acc1s):.4f}±{np.std(sp_acc1s):.4f}  bAcc={np.mean(sp_baccs):.4f}±{np.std(sp_baccs):.4f}  F1={np.mean(sp_f1s):.4f}±{np.std(sp_f1s):.4f}  (n={SP_N_SEEDS} seeds)")
    if bp_baccs:
        lbl0, lbl1 = LABELS["bp"]
        print(f"BP:  {lbl0}={np.mean(bp_acc0s):.4f}±{np.std(bp_acc0s):.4f}  {lbl1}={np.mean(bp_acc1s):.4f}±{np.std(bp_acc1s):.4f}  bAcc={np.mean(bp_baccs):.4f}±{np.std(bp_baccs):.4f}  F1={np.mean(bp_f1s):.4f}±{np.std(bp_f1s):.4f}  (n={SP_N_SEEDS} seeds)")
